...
class DiscussChannel(models.Model):
    ...
    def connect_sms_sender_options(self):
        ...
        Twilio numbers only: a verified caller ID is voice-only and offering
        it hands the agent a line whose messages are never delivered.
        """
        ...
        return {'default': CallerId._get_default_number(), 'options': options}
    ...
    def _connect_sms_sender(self):
        ...
        Message = self.env['connect.message'].sudo()
        last_inbound = Message.search(
            [('channel_id', '=', self.id), ('status', '=', 'received')],
            order='create_date desc', limit=1)
        if not last_inbound and self.connect_number:
            # connect.message only started carrying channel_id recently, so
            # the inbound history of a conversation older than that is not
            # linked to it. Match on the customer's number instead, or those
            # conversations all fall through to the default line.
            last_inbound = Message.search(
