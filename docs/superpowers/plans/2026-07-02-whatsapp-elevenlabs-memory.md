...
Note: creating a real `connect.elevenlabs_agent` needs the ElevenLabs API + a valid license, so the test passes a placeholder `browse(<id>)` recordset — the helper only reads `agent.id`. `connect.whatsapp_sender.create({"number": ...})` works with just the number.
