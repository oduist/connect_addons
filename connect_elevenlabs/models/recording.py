...
class Recording(models.Model):
    ...
    elevenlabs_transcription = fields.Text(readonly=True)
    elevenlabs_summary = fields.Text(readonly=True)
    elevenlabs_media_file = fields.Binary()
    if release.version_info[0] >= 17.0:
        elevenlabs_recording_widget = fields.Html(compute='_elevenlabs_recording_widget', string='Recording',
                                                  sanitize=False)
    else:
        elevenlabs_recording_widget = fields.Char(compute='_elevenlabs_recording_widget', string='Recording')
